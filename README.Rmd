...
devtools::install_github("philstraforelli/forcatsmisc")
```
...
One thing I've been doing more and more often in a shiny app is to place the scales of a factor variable using [shinyWidgets::sliderTextInput()](https://github.com/dreamRs/shinyWidgets#slider-text). The `lvl_range()` was created to make it easier to handle the output of the UI function on the backend (such as when filtering the data to include only the range of factor levels selected).

![sliderTextInput()](https://raw.githubusercontent.com/dreamRs/shinyWidgets/master/inst/images/sliderText.png)
